...
- **Typography Excellence (Font Versions)** — Use Display versions for big headlines only, Text versions for long bodies, Caption/Micro versions for small short texts (1-2 lines). Don't worship legibility — pick fonts that trigger emotion for headlines. Kerning: bigger text = smaller kerning (-4 for large, 0 for average, +4 for very small). Use mathematical relationships between sizes (golden ratio or major third). *(source: nextjs-claude-design-skill — UX Designer)*
- **Sophisticated Palette Strategy** — Prefer subtle, slightly desaturated colours that feel "photographed" rather than "rendered". Use neutral pastel palettes with accent colours for focus. Warm greys as base tones. Limit accent colours to guide attention. Don't default to generic SaaS blue (#3B82F6). *(source: nextjs-claude-design-skill — UX Designer)*
- **Layout Composition & Attention Guidance** — Every screen must feel balanced in visual weight and negative space. Use generous negative space to focus attention. Grid/flex wrappers with `gap` for spacing — prioritise wrappers over direct margins/padding. Design interfaces that guide user attention effectively with clear paths through content. *(source: nextjs-claude-design-skill — UX Designer)*
- **Stand Out From Generic Patterns** — Avoid "Claude style" designs (excessive bauhaus, liquid glass, apple-like). Don't use generic SaaS aesthetics. Draw inspiration from modern landing pages (Perplexity, Comet, Dia), Framer templates, leading brand studios, historical movements (Bauhaus, Otl Aicher, Braun) as inspiration not imitation. Suggest photography, patterns, textures — not just solid colours. *(source: nextjs-claude-design-skill — UX Designer)*
</skills>
