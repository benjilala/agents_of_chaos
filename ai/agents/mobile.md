...
- **Mobile-First Responsive Patterns** — Start with mobile design, progressively enhance for larger screens. Use relative units (%, em, rem) instead of fixed pixels. Fluid typography with `clamp()`. Responsive image `sizes` attribute for optimal loading. Test at 375px (iPhone SE), 390px (iPhone 12 Pro), 768px (iPad), 1024px (iPad Pro). *(source: nextjs-claude-design-skill — UX Designer / RESPONSIVE-DESIGN.md)*
- **Touch-Friendly Forms** — Full-width inputs on mobile, side-by-side on desktop. `touch-manipulation` on all form inputs to prevent 300ms tap delay. Full-width submit button on mobile (`w-full md:w-auto`). Stack form fields vertically below 768px. *(source: nextjs-claude-design-skill — UX Designer / RESPONSIVE-DESIGN.md)*
- **Responsive Navigation** — Mobile menu with burger toggle (hidden at `md:`), full-screen overlay nav on mobile, horizontal `flex gap-6` on desktop. Sticky header with `bg-white/80 backdrop-blur-md`. Skip links with `sr-only focus:not-sr-only`. *(source: nextjs-claude-design-skill — UX Designer / RESPONSIVE-DESIGN.md)*
- **Responsive Content Strategy** — Show/hide content per breakpoint (`block md:hidden`, `hidden md:block`). Snap scrolling for horizontal carousels (`snap-x snap-mandatory`). `overscroll-contain` to prevent bounce. Lazy load below-fold images. Use `aspect-video` for responsive media. *(source: nextjs-claude-design-skill — UX Designer / RESPONSIVE-DESIGN.md)*
</skills>
