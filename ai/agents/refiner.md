...
- **Design Implementation Standards** — Spec must reference: shadcn components for UI primitives, Tailwind utility classes for styling, grid/flex with `gap` for layout, `clsx` for conditional classes, `sonner` for toasts, skeleton loading states for every async component, `touch-manipulation` on all form inputs, `content-visibility: auto` for long lists. *(source: nextjs-claude-design-skill — UX Designer)*
- **Responsive Spec Requirements** — Every component in the spec must define responsive behaviour: mobile-first breakpoints (base → sm → md → lg), touch target sizes (≥44×44px), thumb zone placement for primary CTAs, safe area consideration for notch devices, fluid typography with `clamp()` where applicable. *(source: nextjs-claude-design-skill — UX Designer / RESPONSIVE-DESIGN.md)*
- **Accessibility Spec Requirements** — Every component must specify: keyboard navigation path, focus management (especially for overlays/modals), ARIA attributes needed, colour contrast compliance, `prefers-reduced-motion` fallback, and `sr-only` content for screen readers. Reference WCAG 2.1 AA as minimum bar. *(source: nextjs-claude-design-skill — UX Designer / ACCESSIBILITY.md)*
</skills>
