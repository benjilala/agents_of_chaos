...
- **"Swap the Brand" Uniqueness Test** — Apply this litmus test to every design: could you put a competitor's logo on this and it still works? If yes, it's generic and fails. Check for: generic SaaS blue (#3B82F6), excessive liquid glass, Apple mimicry, machine-generated aesthetics. The design must be distinctly *this* product. *(source: nextjs-claude-design-skill — UX Designer)*
- **Common Anti-Patterns Checklist** — Systematic check against known bad patterns: default shadows/gradients for depth, Apple design language copying, glass morphism effects, design decisions without asking, typography without considering font version, animations that delay user actions, cluttered interfaces with competing elements, generic training dataset aesthetics. *(source: nextjs-claude-design-skill — UX Designer)*
</skills>
