...
- **ARIA Patterns Library** — Reference patterns for: accessible modals (`role="dialog"`, `aria-modal="true"`, `aria-labelledby`), accessible tabs (`role="tablist"`, `aria-selected`, `aria-controls`), accessible tooltips (`aria-describedby`, show on focus+hover), ARIA live regions (`role="status"` polite, `role="alert"` assertive), expanded/pressed/checked states. *(source: nextjs-claude-design-skill — UX Designer / ACCESSIBILITY.md)*
- **Focus Trap in Modals** — When modal opens: store previous focus, move focus to modal, trap Tab within focusable elements (cycle first↔last), return focus to trigger on close. Use `querySelectorAll('button, [href], input, select, textarea, [tabindex]:not([tabindex="-1"])')` for focusable element detection. *(source: nextjs-claude-design-skill — UX Designer / ACCESSIBILITY.md)*
- **Screen Reader Content (sr-only)** — Use `sr-only` class for context that screen readers need but visual users don't (button descriptions, result counts, skip links). Pair with `focus:not-sr-only` for skip links. Every icon-only button must have sr-only text or aria-label. *(source: nextjs-claude-design-skill — UX Designer / ACCESSIBILITY.md)*
- **Colour Contrast Testing** — Normal text: 4.5:1 minimum. Large text (18pt+ or 14pt+ bold): 3:1. UI components and graphics: 3:1. Never rely on colour alone to convey information — always pair with icons, text, or pattern. *(source: nextjs-claude-design-skill — UX Designer / ACCESSIBILITY.md)*
</skills>
